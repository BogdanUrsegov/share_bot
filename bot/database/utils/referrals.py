...
async def get_referral_link(code: str) -> ReferralLink | None:
    code = code.lstrip("#")
    async with AsyncSessionLocal() as session:
        return await session.scalar(select(ReferralLink).where(ReferralLink.code == code))
...
async def record_referral_click(code: str, telegram_id: int, language_code: str | None, is_premium: bool) -> bool:
    code = code.lstrip("#")
    async with AsyncSessionLocal() as session:
        link = await session.scalar(select(ReferralLink).where(ReferralLink.code == code))
        if not link:
